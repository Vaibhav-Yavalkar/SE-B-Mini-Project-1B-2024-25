...
class Database:
    def __init__(self):
        ...
        # Create table to store food waste logs
        self.cursor.execute("""
        ...
        """)
        # Create table to store food stock
        self.cursor.execute("""
    ...
    def add_to_stock(self, food_item, quantity):
        # Check if the food item is already in stock
        ...
        if result:
            ...
            # Insert new food item into stock
    ...
    def reduce_stock(self, food_item, quantity):
        # Reduce the quantity of the food item in stock
        self.cursor.execute("SELECT quantity FROM stock WHERE food_item=?", (food_item,))
        result = self.cursor.fetchone()
        if result:
            new_quantity = result[0] - quantity
...
class FoodWasteApp:
    ...
    def log_food_waste(self):
        ...
        if food_item and quantity.isdigit() and reason:
            ...
            # Insert log into the database
            self.db.insert_log(food_item, quantity, reason)
            # Reduce stock after logging food waste
